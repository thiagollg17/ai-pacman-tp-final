...
def crossover(rejectionsampling):          #indices en la poblacion que haran de padres
    global weights
    ...
    for child in range(population_size):  # n_generation
        ...
        while len(padres) <2:
          ...
          for weight in weights.values():
              ...
              fitnessactual = random.choice(rejectionsampling)
              r = random.random()
              if r< fitnessactual:
                  if len(padres) < 2:
                      padres.extend([weight])
                  elif len(padres) == 2:
...
def main():
    ...
    for generation in range(n_generations):
        ...
        crossover(allFitness)  # nuestra reproduction
        allFitness =  [0] * population_size
    # thiago
