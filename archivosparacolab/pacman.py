...
pygame.mixer.quit()  # Desactiva el módulo de audio
pygame.init()
import numpy as np
...
# Define un evento personalizado para cuando Pacman esté detenido
PACMAN_STOPPED_EVENT = pygame.USEREVENT + 1
...
class game():
    ...
    def gethiscores(self):
        ...
        try:
            f = open(os.path.join(SCRIPT_PATH, "images", "hiscore.txt"))
            hs = []
            for line in f:
    ...
    def writehiscores(self, hs):
        ...
        fname = os.path.join(SCRIPT_PATH, "images", "hiscore.txt")
        f = open(fname, "w")
        for line in hs:
...
class path_finder():
    ...
    def draw(self, row_col):
        for row in range(0, self.size[0], 1):
            for col in range(0, self.size[1], 1):
                thisTile = self.GetType(row_col)
...
class ghost():
    ...
    def FollowNextPathWay(self, max_recursions=10, current_recursion=0):
    # Limitar el número de recursiones
      if current_recursion >= max_recursions:
          print("Recursión máxima alcanzada. Deteniendo.")
          return

      # Solo sigue este camino si hay un camino disponible
      if self.currentPath != False:
          if len(self.currentPath) > 0:
              ...
              # Este fantasma ha alcanzado su destino!!
              if self.state != 3:
                  # Persigue a Pac-Man
                  self.currentPath = path.FindPath((self.nearestRow, self.nearestCol),
                                                  (player.nearestRow, player.nearestCol))
                  self.FollowNextPathWay(max_recursions, current_recursion + 1)

              else:
                  # Ha encontrado el camino de vuelta a la caja de fantasmas
                  self.state = 1
                  self.speed = self.speed / 4

                  # Da al fantasma un camino a un lugar aleatorio con un pellet
                  randRow, randCol = 0, 0
    ...
    def __init__(self):
        ...
        for i in range(0, 5, 1):
            self.imFruit[i] = pygame.image.load(
                os.path.join(SCRIPT_PATH, "images", "sprite", "fruit " + str(i) + ".gif")).convert()
...
class pacman():
    ...
    def __init__(self):
        ...
        # Otros atributos
        #self.current_direction = 5  # -1 como valor inicial para indicar sin movimiento
        ...
        # Inicializa el historial de movimientos como tres movimientos "vacíos"
        self.movement_history = [5, 5, 5]
    ...
    def initialize_visit_map(self, height, width):
        # Crear `visit_map` después de que `thisGame` esté disponible
        self.visit_map = [[0 for _ in range(height)] for _ in range(width)]

    def check_if_stopped(self):
        if self.velX == 0 and self.velY == 0:
            pygame.event.post(pygame.event.Event(PACMAN_STOPPED_EVENT))

    def update_movement(self):
        # Solo intentamos obtener un nuevo movimiento si Pacman está detenido
        if self.velX == 0 and self.velY == 0:
            ghostDirection = {"R": [1, 0, 0, 0], "L": [0, 1, 0, 0], "U": [0, 0, 1, 0], "D": [0, 0, 0, 1]}
            d_ghosts = [None] * 4
            ghostDistance = [None] * 4

            # Calcula las distancias y direcciones de los fantasmas
            for i in range(4):
                ghosts[i].Move()
            ...
            # Lista de direcciones intentadas para evitar bucles infinitos
            attempted_directions = set()
            all_directions = [0, 1, 2, 3]  # 0: Arriba, 1: Abajo, 2: Izquierda, 3: Derecha

            while True:
                # Genera la entrada para la red neuronal y obtiene una dirección sugerida
                #input_ga = GAInput(ghostDistance, ghostDirections, self, movement_history=self.movement_history)
                # Inicialización del mapa de visitas
                input_ga = GAInput(ghostDistance, self)
                new_direction = ga.neural_net(thisPopulation, input_ga)
                #attempted_directions.add(new_direction)
                # Si ya intentamos esta dirección, selecciona otra
                if new_direction in attempted_directions:
                    new_direction = random.choice([d for d in all_directions if d not in attempted_directions])

                # Añade la dirección actual a las direcciones intentadas
                attempted_directions.add(new_direction)

                # Asigna la dirección sugerida a las velocidades temporales
                temp_velX, temp_velY = 0, 0
                if new_direction == 0:  # Arriba
                    temp_velX, temp_velY = 2, 0
                elif new_direction == 1:  # Abajo
                    temp_velX, temp_velY = -2, 0
                elif new_direction == 2:  # Izquierda
                    temp_velX, temp_velY = 0, -2
                elif new_direction == 3:  # Derecha
                    temp_velX, temp_velY = 0, 2
                ...
                if not thisLevel.CheckIfHitWall((new_x, new_y), (self.nearestRow, self.nearestCol)):
                    # Si no intercepta un muro, aplica las velocidades
                    self.velX, self.velY = temp_velX, temp_velY
                    if self.velX == 2 and self.velY == 0:
                        self.current_direction = 0  # right
                    elif self.velX == -2 and self.velY == 0:
                        self.current_direction = 1  # left
                    elif self.velX == 0 and self.velY == -2:
                        self.current_direction = 2  # down
                    elif self.velX == 0 and self.velY == 2:
                        self.current_direction = 3  # up
                    # Actualiza el historial de movimientos con el nuevo movimiento válido
                    #if len(self.movement_history) >= 3:
                   #     self.movement_history.pop(0)  # Elimina el movimiento más antiguo
                    #self.movement_history.append(new_direction)

                    # Obtener la columna y la fila actual
                    col = int(self.x / 16) -1 # Coordenada x en términos de celdas
                    row = int(self.y / 16) -1 # Coordenada y en términos de celdas

                    # Incrementar el contador en la celda actual del mapa de visitas
                    #self.visit_map[col][row] += 1

                    # Sal del bucle
                    break

                # Si todos los movimientos han sido intentados y fallaron, elige permanecer en su lugar
                if len(attempted_directions) == 4:
                    self.velX, self.velY = 0, 0
                    break

        # Actualiza la posición de Pacman en base a las velocidades actuales
        self.x += self.velX
        self.y += self.velY
    ...
    def Move(self):
        ...
        # Determinar la dirección del movimiento basado en velX y velY
        if self.velX == 2 and self.velY == 0:
            self.current_direction = 0  # right
        elif self.velX == -2 and self.velY == 0:
            self.current_direction = 1  # left
        ...
        if not thisLevel.CheckIfHitWall((self.x + self.velX, self.y + self.velY), (self.nearestRow, self.nearestCol)):
            #if thisLevel.CheckIfHit((self.x, self.y), (ghosts[0].x, ghosts[i].y), 8):
        #if boca ==1 :
            # it's ok to Move
            self.x += self.velX
            self.y += self.velY
            ...
            #if self.current_direction != 5:  # Solo actualiza si hubo movimiento
            #    if len(self.movement_history) >= 3:
            #        self.movement_history.pop(0)  # Eliminar el movimiento más antiguo si hay más de 3
            #    self.movement_history.append(self.current_direction)  # Añadir el nuevo movimiento
...
class level():
    ...
    def Restart(self):
        ...
        for i in range(0, 4, 1):
            ...
            ghosts[i].velY = 0
            ghosts[i].state = 1
            ghosts[i].speed = 1 #thiago original 1
            ghosts[i].Move()
...
# Detecta si Pacman está detenido y llama a update_movement
        if event.type == PACMAN_STOPPED_EVENT:
            # Llama a `update_movement` cuando Pacman está detenido
 ...
 #   allDots = []
  #  for row in range(-1, thisGame.screenTileSize[0] + 1):
    ...
    #        actualCol = thisGame.screenNearestTilePos[1] + col
            ...
            # Verifica el tipo de tile
     #       useTile = thisLevel.GetMapTile((actualRow, actualCol))
     #       if useTile == tileID['pellet']:  # Solo pastillas normales
     #           allDots.append((actualRow, actualCol))

    # Si no hay pacdots, devuelve None
   # if not allDots:
    #    return None
    ...
    # Encuentra el pacdot más cercano ordenando por distancia euclidiana
    #closestDot = min(allDots, key=lambda p: distance.euclidean(curLocation, p))
...
def direction_to_one_hot(direction):
    one_hot = [0, 0, 0, 0]
    if 0 <= direction < 4:  # Verifica que la dirección esté en el rango
        one_hot[direction] = 1
    return one_hot

from scipy.spatial import distance

def GAInput(ghostDistance, playerinput):
    # Normalizar las distancias a los fantasmas
    walls = GetWallInput()
    #hastachocar = get_all_directions_distance(playerinput.x,playerinput.y)
    ghostDistance = [float(i) / 30 for i in ghostDistance]
    orientacion = direction_to_one_hot(playerinput.current_direction)
    # Determinar la dirección de animación actual
    #if playerinput.anim_pacmanCurrent == playerinput.anim_pacmanU:
    #    anim_direction = 3
    #elif playerinput.anim_pacmanCurrent == playerinput.anim_pacmanD:
    #    anim_direction = 2
    #elif playerinput.anim_pacmanCurrent == playerinput.anim_pacmanL:
    #    anim_direction = 1
    #elif playerinput.anim_pacmanCurrent == playerinput.anim_pacmanR:
    #    anim_direction = 0
    #else:
    #    anim_direction = -1  # Valor por defecto si no coincide con ninguna dirección

    # Obtener la distancia al pacdot más cercano
    #closestPacdot = GetClosestPacdot()
    #pacdot_distance = distance.euclidean((playerinput.x, playerinput.y), closestPacdot) if closestPacdot else -1

    # Actualizar el mapa de visitas para la celda actual
    #row, col = int(playerinput.y / 16), int(playerinput.x / 16)
    #playerinput.visit_map[row][col] += 1

    # Obtener la cantidad de visitas de las celdas adyacentes
    #adjacent_visits = [
    #    playerinput.visit_map[row - 1][col] if row - 1 >= 0 else 0,  # Arriba
    #    playerinput.visit_map[row + 1][col] if row + 1 < len(playerinput.visit_map) else 0,  # Abajo
    #    playerinput.visit_map[row][col - 1] if col - 1 >= 0 else 0,  # Izquierda
    #    playerinput.visit_map[row][col + 1] if col + 1 < len(playerinput.visit_map[0]) else 0  # Derecha
    #]

    # Contar la cantidad de pacdots en las celdas adyacentes
    #adjacent_pacdots = [
    #    GetPacdotsInCell(row - 1, col) if row - 1 >= 0 else 0,  # Arriba
    #    GetPacdotsInCell(row + 1, col) if row + 1 < len(playerinput.visit_map) else 0,  # Abajo
    #    GetPacdotsInCell(row, col - 1) if col - 1 >= 0 else 0,  # Izquierda
    #    GetPacdotsInCell(row, col + 1) if col + 1 < len(playerinput.visit_map[0]) else 0  # Derecha
    #]

    # Crear el vector de entrada, incentivando exploración
    inputValues = walls + ghostDistance + orientacion
...
#def GetPacdotsInCell(row, col):
    # Asegurarse de que la posición está dentro de los límites del mapa
#if 0 <= row < thisGame.screenTileSize[0] and 0 <= col < thisGame.screenTileSize[1]:
        # Obtener el tipo de tile en la posición (row, col)
      #  tile_type = thisLevel.GetMapTile((row, col))

        # Comprobar si el tile es un pacdot
      #  if tile_type == tileID['pellet']:
      #      return 1  # Hay un pacdot en esta celda
       # else:
      #      return 0  # No hay pacdot en esta celda
   # else:
     #   return 0  # Si la celda está fuera del mapa, retorna 0

def get_all_directions_distance(x, y):
    ...
    for direction in directions:
        ...
        # Inicializa la distancia y las coordenadas
        distance = 0
        current_x, current_y = x, y

        # Calcula la distancia hasta que se encuentra un obstáculo o una pared
        while True:
            # Verificar si se encuentra un fantasma
            if thisLevel.CheckIfHit((current_x, current_y), (int(current_y / 16), int(current_x / 16)), cushion=16):
                break

            # Verificar si se encuentra una pared
            if thisLevel.CheckIfHitWall((current_x, current_y), (int(current_y / 16), int(current_x / 16))):
                break

            # Si no encontró nada, sigue moviéndose en la dirección dada
            current_x += dx * 16
            current_y += dy * 16
            distance += 1
    ...
    # Normalizar las distancias en el rango de 0 a 1
    max_distance = 24
    normalized_distances = [(dist / max_distance)*10 for dist in distances]

    return normalized_distances

def get_wall_distance(x, y, direction):
    # Define el incremento en x e y basado en la dirección
    dx, dy = 0, 0
    if direction == "up":
        dy = -1
    elif direction == "down":
        ...
        raise ValueError("Dirección inválida: elija 'up', 'down', 'left' o 'right'.")

    # Inicializar la distancia y la posición actual
    distance = 0
    current_x, current_y = x, y

    # Incrementar la posición en la dirección dada hasta encontrar una pared
    while not thisLevel.CheckIfHitWall((current_x, current_y), (int(current_y / 16), int(current_x / 16))):
        current_x += dx * 16  # Ajusta la distancia en píxeles (16 es el tamaño de una celda)
        current_y += dy * 16
        distance += 1
...
def CheckInputs(direction, mode_game):
    ...
    if mode_game == 3:
        ...
        global thisPopulation   #poblacion actual
        global allFitness




        # input_ga = GAInput(ghostDistance, ghostDirections, player, movement_history=player.movement_history)
        input_ga2 = GAInput(ghostDistance, player)
...
def createpop(size):
    return [pacman() for _ in range(size)]
#      __________________
...
thisLevel.LoadLevel(thisGame.GetLevelNum())
    # thisPopulation = -1
    # thisGeneration = 0
thisPopulation = int(sys.argv[1])
    # thisGeneration = ga.generation
    # allFitness = np.zeros((n_generations, population_size))
    # allFitness[game_id] = np.zeros(population_size)  # Fitness específico para cada instancia de juego
...
while True:
    ...
    if thisGame.mode == 1:
        ...
        #input_ga = GAInput(ghostDistance, ghostDirections, player, movement_history=player.movement_history)
        input_ga = GAInput(ghostDistance, player)
