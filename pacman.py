...
class game():
    ...
    def makehiscorelist(self):
        ...
        f = pygame.font.Font(os.path.join(SCRIPT_PATH, "images", "VeraMoBd.ttf"), 10)
        ...
        if not self.currentPath == False:
            ...
            if len(self.currentPath) > 0:
                ...
                if not self.state == 3:
                    ...
                    self.currentPath = path.FindPath((self.nearestRow, self.nearestCol),
                                                     (player.nearestRow, player.nearestCol))
                    if (thisGame.recursive < 5):
                        thisGame.recursive += 1
                        self.FollowNextPathWay()
                    ...
                    # give ghost a path to a random spot (containing a pelletpellet)
                    (randRow, randCol) = (0, 0)
...
def GAInput(ghostDistance, playerinput):
    ...
    orientacion = direction_to_one_hot(player.current_direction)
...
#change thiago
        ...
        fitness = ga.cal_pop_fitness(thisGame.score, (thisGame.elapsedTime + 1.0e-21) / 1000, input_ga2[4:8])
        ...
        mating_pool = []
        max_fitness = find_max_fitness(allFitness[thisGeneration])
        ...
        if (thisPopulation >= population_size):
            ...
            index = 0
            weight = []
            for weight in ga.weights.values():
                # Obtener la aptitud del individuo
                fitness = allFitness[thisGeneration][index]

                # Normalizar la aptitud entre 0 y 1
                fitness_normal = map_value(fitness, 0, max_fitness, 0, 1)
                ...
                mating_pool.extend([weight] * n)  # Agregar el individuo n veces
...
#funciona
            ...
            ga.crossover(mating_pool) # nuestra reproduction
...
player.initialize_visit_map(thisLevel.lvlHeight, thisLevel.lvlWidth)
...
while True:
    ...
    if thisGame.mode == 1:
        ...
        for i in range(0, 4, 1):
            ghosts[i].Move()
            # Verifica si currentPath tiene elementos antes de acceder al último
            d_ghosts[i] = ghosts[i].currentPath[-1] if ghosts[i].currentPath else ''
            # print(d_ghosts)
            ghostDistance[i] = len(ghosts[i].currentPath)
        ghostDirections = (ghostDirection[d_ghosts[0]], ghostDirection[d_ghosts[1]], ghostDirection[d_ghosts[2]],
                           ghostDirection[d_ghosts[3]])
